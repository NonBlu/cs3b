...
//Lab 15
//Purpose: Write a program that reads int values from input.txt into an array
...
ldr     x4,=dbNumArray //obtain address of array and store it into x4
top1: 
    ...
    //else
    ldr     x0,=fileBuf //fileBuf contains contents of one line of the input

    //retrieved value has a newline at the end

    //remove that newline
    //find length of the string, then increment string by that amount
    bl      String_length 
    sub     x0,x0,#1
    //now x0 contains string length
    //store into x3
    mov     x3,x0 
    ...
    strb    w2,[x0] //store that into x0
    sub     x0,x0,x3
    
    //convert string value to quad
    bl      ascint64

    //NOW STORE INTO ARRAY
    //increment x4 by how many inputs we have received
    str     x0,[x4] //Store quad value into x4
